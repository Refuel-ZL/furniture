    ...
    fetchproall: async function(params) {
        ...
        if (!params) {
            ...
            try {
                ...
                let sql1 = `SELECT SQL_CALC_FOUND_ROWS oif.pid,DATE_FORMAT( oif.regtime, '%Y-%m-%d %H:%i:%s') as regtime,oif.status,oif.category FROM orderinfo AS oif ${valsql} order by oif.${params.sortName} ${params.sortOrder} ${LIMIT} `
